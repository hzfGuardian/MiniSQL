...
//cout<<"-------------------------------------------------------------"<<endl;
//for(int i=0;i<14;i++)
//	cout<<p->getvalue(i)<<" "<<p->getadree(i)<<endl;
//cout<<"-------------------------------------------------------------"<<endl;
	return p;
}
...
int parent(int p,int root,string filename) //找p的父节点 
{
	...
	string k=getnode(p,filename)->getvalue(0);
	while(root!=p)
	{
...
int insert(string k,int off,int place,string filename){
	...
	else //1.叶节点，但是满了 2.非叶节点
	{
		...
		else //满了 （1.一开始的满叶节点 2.找到所在的叶节点满了）
		{
			...
			int ot=insertpar(newtree->getvalue(0),newp,place,t,filename);	
			return ot;
		}
	}
}
...
//	while(!(p_parent->getvalue(i)==k) && i<N-1)
	...
	else changepar(p_p,k,r,k2,filename);		
}
